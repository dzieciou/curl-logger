...
public class Http2CurlTest {
  ...
  public void shouldPrintGetRequestProperly() throws Exception {
    ...
    assertThat(
        getNonWindowsHttp2Curl().generateCurl(getRequest),
  ...
  public void shouldNotPrintInvalidBasicAuthnUserCredentials() throws Exception {
    ...
    assertThat(
        getNonWindowsHttp2Curl().generateCurl(getRequest),
        equalTo(
            "curl 'http://test.com:8080/items/query?x=y#z' -H 'Authorization: Basic xxx'"
                + " --compressed -k -v"));
  }
  ...
  public void shouldPrintPostRequestProperly() throws Exception {
    ...
    assertThat(
        getNonWindowsHttp2Curl().generateCurl(postRequest),
        equalTo(
  ...
  public void shouldPrintDeleteRequestProperly() throws Exception {
    ...
    assertThat(
        getNonWindowsHttp2Curl().generateCurl(deleteRequest),
  ...
  public void shouldPrintHeadRequestProperly() throws Exception {
    ...
    assertThat(
        getNonWindowsHttp2Curl().generateCurl(headRequest),
  ...
  public void shouldPrintMultipleCookieHeadersInMultipleParameters() throws Exception {
    ...
    assertThat(
        getNonWindowsHttp2Curl().generateCurl(headRequest),
        equalTo(
            "curl 'http://test.com/items/12345' -X HEAD -H 'Cookie: X=Y; A=B' -H 'Cookie: D=E'"
                + " --compressed -k -v"));
  }
  ...
  public void shouldPrintPutRequestProperly() throws Exception {
    ...
    assertThat(
        getNonWindowsHttp2Curl().generateCurl(putRequest),
        equalTo(
            "curl 'http://test.com/items/12345' -X PUT -H 'Content-Type: application/json'"
                + " --data-binary 'details={\"name\":\"myname\",\"age\":\"20\"}' --compressed -k"
                + " -v"));
  }
  ...
  public void shouldPrintMultilineRequestProperly() throws Exception {
    ...
    // when
    Options options =
        Options.builder().targetPlatform(Platform.UNIX).useShortForm().printMultiliner().build();

    // then
    assertThat(
        new Http2Curl(options).generateCurl(postRequest),
        equalTo(
            "curl 'http://google.pl/' \\\n"
  ...
  public void shouldWriteParametersInLongForm() throws Exception {
    ...
    // when
    Options options =
        Options.builder().targetPlatform(Platform.UNIX).useLongForm().printSingleliner().build();

    // then
    assertThat(
        new Http2Curl(options).generateCurl(getRequest),
        equalTo(
            "curl 'http://test.com:8080/items/query?x=y#z' --header 'Host: H' --compressed"
                + " --insecure --verbose"));
  }
  ...
  public void shouldPrintPostRequestWithMethodProperly() throws Exception {
    ...
    assertThat(
        new Http2Curl(options).generateCurl(postRequest),
        equalTo(
            "curl 'http://google.pl/' -X POST -H 'Content-Type: application/x-www-form-urlencoded'"
                + " --data-binary 'param1=param1_value' --compressed -k -v"));
  }
