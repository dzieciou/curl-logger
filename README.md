...
NOTE: The library updates `httpClient` attribute ot `RestAssuredConfig` instance, so your changes to this attribute will be overwritten. Also, if you modify `htttpClient` attribute after configuring curl-logger, curl-logger will no longer log requests.
